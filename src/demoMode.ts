...
const DEMO_INSTALLED_BUILD = {
  version: '1.7.10',
  versionTag: 'v1.7.10',
  installedIdentity: 'v1.7.10 / demo-build',
  sourceRef: 'v1.7.10',
  commit: 'demo-build',
  buildDate: '2026-08-25',
  containerImage: 'ghcr.io/virtuarchitect/ztf-orchestrator:v1.7.10',
...
async function demoResponse(request: Request) {
  ...
  if (path === '/health') return json({ status: 'healthy', version: '1.7.10', installed: DEMO_INSTALLED_BUILD, storage: 'demo' })
  ...
  if (path === '/api/appliance/status') {
    return json({
      detected: true,
      runtime: { status: 'healthy', version: '1.7.10', installed: DEMO_INSTALLED_BUILD, ztfCompatible: true, message: 'Legacy ZTF 1.x workflow/script CLI detected' },
  ...
  if (path === '/api/appliance/updates') {
    return json({
      current: { version: '1.7.10', installed: DEMO_INSTALLED_BUILD, containerImage: 'ghcr.io/virtuarchitect/ztf-orchestrator:v1.7.10', requestPath: '/var/lib/ztf-orchestrator/appliance_update_request.json' },
      updates: [],
      staged: null,
