...
async function demoResponse(request: Request) {
  ...
  if (path === '/api/native-foundation/phases') return json({
    ...
    summary: {
      ...
      currentBoundary: 'Static demo data only. Real Dell iDRAC Redfish deployment requires appliance runtime gates and a configured local adapter command.',
    },
  ...
  if (path === '/api/native-foundation/provider-adapters') return json({
    ...
    canRunFullDeployment: false,
    realDeploymentAdapter: {
      enabled: false,
      commandConfigured: false,
      ready: false,
      commandName: '',
      blockedReason: 'Static demo only. Enable the appliance/container adapter command before full Native Foundation deployment.',
    },
    status: 'blocked',
    ...
    providerAdapters: [{
      ...
      mutatingActionsEnabled: false,
      fullDeploymentReady: false,
      deploymentBlockedReasons: ['Static demo only. Use the appliance/container runtime with the bundled Native Foundation adapter command for controlled-UAT deployment.'],
      realDeploymentAdapter: {
        enabled: false,
        commandConfigured: false,
        ready: false,
