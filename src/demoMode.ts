...
async function demoResponse(request: Request) {
  ...
  if (path === '/health') return json({ status: 'healthy', version: '1.7.4', storage: 'demo' })
  ...
  if (path === '/api/appliance/status') {
    return json({
      detected: true,
      runtime: { status: 'healthy', version: '1.7.4', ztfCompatible: true, message: 'Legacy ZTF 1.x workflow/script CLI detected' },
  ...
  if (path === '/api/appliance/updates') {
    return json({
      current: { version: '1.7.4', containerImage: 'ghcr.io/virtuarchitect/ztf-orchestrator:v1.7.4', requestPath: '/var/lib/ztf-orchestrator/appliance_update_request.json' },
      updates: [],
      staged: null,
