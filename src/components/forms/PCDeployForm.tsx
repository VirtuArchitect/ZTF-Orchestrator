...
export default function PCDeployForm({ onYamlChange, profile }: Props) {
  ...
  const [pcVersion, setPcVersion] = useState(profile?.prismCentral.defaultPcVersion || 'pc.2024.3.1.14')
  ...
  useEffect(() => {
    ...
    onYamlChange(buildPCDeployYaml({
      peCredential: peCred, cvmCredential: cvmCred, pcVersion, fileUrl, metadataUrl,
      vmSize, dnsServers, ntpServers, container, clusters,
    }))
  }, [peCred, cvmCred, pcVersion, fileUrl, metadataUrl, vmSize, dnsServers, ntpServers, container, clusters, onYamlChange])
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="form-section">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><label className="label">PC Version</label>
            <input className="input font-mono text-xs" value={pcVersion} onChange={e => setPcVersion(e.target.value)} list="pc-version-options" placeholder="pc.2024.3.1.14" />
            <datalist id="pc-version-options">
              {PC_VERSIONS.map(v => <option key={v} value={v} />)}
            </datalist></div>
      ...
      <div className="space-y-4">
        ...
        {clusters.map((cluster, i) => (
          <div key={i} className="card border-border/70">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div><label className="label text-xs">PC VM Prefix</label><input className="input text-xs py-1.5" value={cluster.pcVmName} onChange={e => updCluster(i, { pcVmName: e.target.value })} placeholder="PC-VM" /></div>
        ...
        <button onClick={() => setClusters(p => [...p, { clusterIp: '', pcVmName: 'PC-VM', pcIp: '', networkName: '', defaultGateway: '', subnetMask: '255.255.255.0', vip: '' }])} className="btn-secondary w-full justify-center gap-2">
