...
```bash
sudo ZTF_ORCHESTRATOR_VERSION=v1.6.0 \
...
```bash
docker manifest inspect ghcr.io/virtuarchitect/ztf-orchestrator:v1.6.0
```
...
### Public v1.6.0 Appliance Image

A prebuilt standard AHV QCOW2 bundle for `v1.6.0` is generated by the AHV
appliance image workflow for the release ref.

```text
GitHub Actions artifact: ztf-orchestrator-ahv-qcow2-standard-v1.6.0
```

Bundle:

```text
ztf-orchestrator-ahv-qcow2-standard-v1.6.0
```

QCOW2:

```text
ztf-orchestrator-appliance-standard-v1.6.0.qcow2
```
...
cd appliance
VERSION=v1.6.0 \
ZTF_ORCHESTRATOR_VERSION=v1.6.0 \
...
packer build \
  -var "version=v1.6.0" \
  -var "ztf_orchestrator_version=v1.6.0" \
