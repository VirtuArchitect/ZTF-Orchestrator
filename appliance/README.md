...
| --- | --- |
| Current release baseline | `v1.8.1` |
...
5. Attach checksum and manifest metadata to a versioned GitHub Release, and
   store the full QCOW2 in Actions artifacts or durable internal storage.
...
```bash
docker manifest inspect ghcr.io/virtuarchitect/ztf-orchestrator:<version>
```
...
Current in-place container update packages are produced for `v1.8.1`. Use the
...
After applying an update, confirm **Settings > About > Installed Build** in the
web UI. The footer only shows the base UI version; Installed Build includes the
version tag, source ref or patch, commit, image tag, and update package metadata
needed to distinguish patched builds.
...
Current GitHub Actions appliance builds publish named appliance artifact
profiles. Older single-image workflow lines are historical and should not be
used for new appliance builds.
...
### Historical Public v1.7.0 Appliance Image

Historical evidence only. Do not use this section as the current appliance
image download path. Build or request a QCOW2 for the current release ref and
archive the resulting Actions artifact in durable storage.

A prebuilt standard AHV QCOW2 bundle for `v1.7.0` was generated by the AHV
appliance image workflow for the release ref.

```text
GitHub Actions artifact: ztf-orchestrator-ahv-qcow2-standard-v1.7.0
```

Bundle:

```text
ztf-orchestrator-ahv-qcow2-standard-v1.7.0
```

QCOW2:

```text
ztf-orchestrator-appliance-standard-v1.7.0.qcow2
```
...
packer build \
  -var "version=<version>" \
  -var "ztf_orchestrator_version=<version>" \
...
- Use [Appliance Update Manager](../docs/appliance-update-manager.md) for
  ...
  checkout changes.
- For AHV appliances built by the GitHub **Build AHV Appliance Image** workflow,
  follow [Appliance Update Manager](../docs/appliance-update-manager.md) for
  current in-place updates. Historical v1.5.x upgrade notes are retained there
