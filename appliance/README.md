...
```bash
sudo ZTF_ORCHESTRATOR_VERSION=v1.5.6 \
...
```bash
docker manifest inspect ghcr.io/virtuarchitect/ztf-orchestrator:v1.5.6
```
...
cd appliance
VERSION=v1.5.6 \
ZTF_ORCHESTRATOR_VERSION=v1.5.6 \
...
packer build \
  -var "version=v1.5.6" \
  -var "ztf_orchestrator_version=v1.5.6" \
