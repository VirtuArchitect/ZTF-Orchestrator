...
| Adapter target connectivity review | Implemented foundation | Records connectivity owner, scope, target allow-list, maintenance window, probe plan, controlled UAT completion requirement, retained-export prerequisite, secret-audit prerequisite, and packet output/export gate summary metadata; the Dell deployment job performs its own gated Redfish target probes before adapter execution. |
...
| Adapter command invocation review | Implemented foundation | Records command owner, catalog, invocation policy, execution identity, output capture, controlled UAT completion requirement, retained-export prerequisite, secret-audit prerequisite, and packet output/export gate summary metadata; the Dell deployment job can invoke a configured real adapter command when the real-adapter env gate is enabled. |
| Adapter output evidence review | Implemented foundation | Records output evidence owner, retention, redaction, failure classification, evidence-store, controlled UAT completion requirement, retained-export prerequisite, secret-audit prerequisite, and packet output/export gate summary metadata; the Dell deployment job persists adapter intent, request, stdout, stderr, and evidence manifest files. |
...
| HCI deployment | Controlled-UAT Dell only | Dell AHV HCI can be submitted only through the real-adapter command path after intent, all-node iDRAC, image, approval, and Prism Element gates pass. |
| Dell XC770 AHV HCI deployment | Controlled-UAT adapter contract | Implemented as a guarded local Foundation adapter command contract; not a general production support certification until bounded hardware UAT evidence is captured and reviewed. |
