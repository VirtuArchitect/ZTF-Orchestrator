# ZTF-Orchestrator UAT

Current release marker: `v1.7.5`.

This folder contains controlled UAT planning and evidence structure.

## Documents

| Document | Purpose |
|---|---|
| [UAT-PLAN.md](UAT-PLAN.md) | Required UAT stages and exit criteria |
| [UAT-CASES.md](UAT-CASES.md) | Baseline UAT scenarios |
| [UAT-EVIDENCE.md](UAT-EVIDENCE.md) | Evidence checklist for UAT records |
| [UAT-RESULTS.md](UAT-RESULTS.md) | Results index and outcome vocabulary |

## Related Existing Docs

- [Operator Controlled UAT Readiness](../operator-controlled-uat-readiness.md)
- [UAT Evidence Checklist](../uat-evidence-checklist.md)
- [Validation Status](../validation-status.md)
- [Sanitized UAT Evidence Record Pattern](../sanitized-uat-evidence-record.md)
